...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagDCout)
    {
        info->event[info->num] = EVENT_DCout;
        info->num++;
        dataPoints->valueDCout = gizStandardDecompressionValue(DCout_BYTEOFFSET,DCout_BITOFFSET,DCout_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueDCout != cur->valueDCout)
    {
        GIZWITS_LOG("valueDCout Changed\n");
        ret = 1;
    }
    ...
    if(last->valueInfTube != cur->valueInfTube)
    {
        GIZWITS_LOG("valueInfTube Changed\n");
        ret = 1;
    }
    if(last->valueInfPyr != cur->valueInfPyr)
    {
        GIZWITS_LOG("valueInfPyr Changed\n");
        ret = 1;
    }
    ...
    if(last->valueInfReciverAdd != cur->valueInfReciverAdd)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueInfReciverAdd Changed\n");
            ret = 1;
        }
    }
    if(last->valueInfReciverCmd != cur->valueInfReciverCmd)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueInfReciverCmd Changed\n");
            ret = 1;
        }
    }
    if(last->valueLightStr != cur->valueLightStr)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLightStr Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(DCout_BYTEOFFSET,DCout_BITOFFSET,DCout_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDCout);
    gizStandardCompressValue(InfTube_BYTEOFFSET,InfTube_BITOFFSET,InfTube_LEN,(uint8_t *)devStatusPtr,dataPoints->valueInfTube);
    gizStandardCompressValue(InfPyr_BYTEOFFSET,InfPyr_BITOFFSET,InfPyr_LEN,(uint8_t *)devStatusPtr,dataPoints->valueInfPyr);
    ...
    devStatusPtr->valueInfReciverAdd = gizY2X(InfReciverAdd_RATIO,  InfReciverAdd_ADDITION, dataPoints->valueInfReciverAdd); 
    devStatusPtr->valueInfReciverCmd = gizY2X(InfReciverCmd_RATIO,  InfReciverCmd_ADDITION, dataPoints->valueInfReciverCmd); 
    ...
    devStatusPtr->valueLightStr = exchangeBytes(gizY2XFloat(LightStr_RATIO,  LightStr_ADDITION, dataPoints->valueLightStr));     
